...
class GUI (QMainWindow):
    ...
    def onModifiedAperture(self, event):
        ...
        if istab > 0:
            ...
            if s.instrument in ['GREAT','HI','HALPHA','VLA','ALMA','SITELLE','IRAM','CARMA','MMA','PCWI','FOREST','WSRT']:
    ...
    def trimCube1D(self,xmin,xmax):
        ...
        if self.specCube.instrument == 'PACS':
            self.specCube.exposure = self.specCube.exposure[xmin:xmax,:,:]
        if self.specCube.instrument in ['FORCAST','SPIRE']:
            self.specCube.eflux = self.specCube.eflux[xmin:xmax,:,:]
            self.specCube.exposure = self.specCube.exposure[xmin:xmax,:,:]
    ...
    def saveCube(self, cont=False):
        ...
        if (fd.exec()):
            ...
            if self.specCube.instrument == 'FIFI-LS':
                ...
                hdul.writeto(outfile,overwrite=True) 
                hdul.close()
            elif self.specCube.instrument == 'SPIRE':
                """Experimental"""
                # Primary header
                hdu = fits.PrimaryHDU()
                hdu.header.extend(self.specCube.header)
                header['NAXIS'] = (3,'Number of axis')
                header['OBJECT'] = (self.specCube.objname, 'Object Name')
                header['REDSHIFT'] = self.specCube.redshift
                # Transform into frequency
                frequency = 299792.458 / self.specCube.wave[::-1]
                cdelt3 = np.nanmean(frequency[1:]-frequency[:-1])
                crpix3 = 1
                crval3 = frequency[0]
                header['CRPIX3'] = (crpix3,'Reference pixel')
                header['CRVAL3'] = (crval3,'Reference pixel value')
                header['CDELT3'] = (cdelt3,'Increment')
                # Extensions
                pixelstd = (self.specCube.pixscale/206264.8)**2 # pixel size in sterad
                ratio = 1.e26 * pixelstd
                hdu1 = self.addExtension(flux[::-1,:,:]/ratio,'image','Jy',header)
                hdu2 = self.addExtension(self.specCube.exposure[::-1,:,:],'coverage',None,header)
                hdu3 = self.addExtension(self.specCube.eflux[::-1,:,:]/ratio,'error',None,header)
                hdul = fits.HDUList([hdu, hdu1, hdu2, hdu3])            
                hdul.writeto(outfile,overwrite=True) 
                ...
                hdul.writeto(outfile,overwrite=True) 
                hdul.close()
            elif self.specCube.instrument == 'MMA':
                header['NAXIS'] = (3,'Number of axis')
                c = 299792.458  # speed of light in km/s 
                header['INSTRUME'] = (self.specCube.header['INSTRUME'], 'Instrument')
                #header['BMAJ'] = self.specCube.header['BMAJ']
                #header['BMIN'] = self.specCube.header['BMIN']
                #header['BPA'] = self.specCube.header['BPA']
                header['BSCALE'] = self.specCube.header['BSCALE']
                header['BZERO'] = self.specCube.header['BZERO']
                header['FREQ0'] = self.specCube.header['FREQ0']
                #header['ALTRVAL'] = self.specCube.header['ALTRVAL']
                #header['ALTRPIX'] = self.specCube.header['ALTRPIX']
                header['CTYPE3'] = self.specCube.header['CTYPE3']
                header['CRPIX3'] = self.specCube.header['CRPIX3']
                ...
                hdul.writeto(outfile,overwrite=True) 
                hdul.close()
            elif self.specCube.instrument == 'WSRT':
                header['NAXIS'] = (3,'Number of axis')
                c = 299792458.0  # speed of light in m/s 
                ...
                header['BMAJ'] = self.specCube.header['BMAJ']
                header['BMIN'] = self.specCube.header['BMIN']
                header['BUNIT'] = ('Jy/Beam', 'Flux units')
                header['FREQR'] = self.specCube.header['FREQR']
                header['CTYPE3'] = ('FREQ', 'PRIMARY AXIS NAME')
                header['CRPIX3'] = self.specCube.header['CRPIX3']
                header['CRVAL3'] = self.specCube.header['CRVAL3']
                header['CDELT3'] = self.specCube.header['CDELT3']
                header['CUNIT3'] = self.specCube.header['CUNIT3']
                header['DATE-OBS'] = self.specCube.header['DATE-OBS']
                header['VELR'] = self.specCube.header['VELR']
                # Flip the cube before saving to order in frequency
                if self.specCube.header['CDELT3'] > 0:
                    flux = np.flip(flux, axis=0) * self.specCube.npix_per_beam
                else:
                    flux *= self.specCube.npix_per_beam
                hdu = fits.PrimaryHDU(flux)
                hdu.header.extend(header)
                hdul = fits.HDUList([hdu])
                hdul.writeto(outfile,overwrite=True) 
    ...
    def loadFile(self, infile):
        ...
        elif self.specCube.instrument in ['MUSE','GREAT','HI','HALPHA','VLA','ALMA','SITELLE','IRAM','CARMA','MMA','PCWI','FOREST','WSRT']: 
            self.bands = ['Flux']
            self.spectra = ['All','Pix']
    ...
    def slideCube(self, event):
       ...
       # Display channel n of the spectral cube
        if self.specCube.instrument in ['GREAT','HI','HALPHA','MUSE',
                                        'VLA','ALMA','SITELLE','IRAM','CARMA','MMA','PCWI','FOREST','WSRT']:
            imas = ['Flux']
    ...
    def updateImages(self):
        ...
        if self.specCube.instrument in ['GREAT','HI','HALPHA','VLA','ALMA',
                                        'MUSE','SITELLE','IRAM','CARMA','MMA','PCWI','FOREST','WSRT']:
            imas = ['Flux']
        elif self.specCube.instrument in ['PACS','FORCAST','SPIRE']:
            imas = ['Flux','Exp']
        elif self.specCube.instrument == 'FIFI-LS':
    ...
    def repairSpectrum(self):
        ...
        for i in range(self.specCube.nx):
            for j in range(self.specCube.ny):
                ...
                if self.specCube.instrument in ['PACS','FIFI','SPIRE']:
